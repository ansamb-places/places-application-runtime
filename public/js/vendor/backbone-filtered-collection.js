...
function Filtered(superset) {
 ...
 // Save a reference to the original collection
  ...
  self._collection = new Backbone.Collection(superset.toArray());
  proxyCollection(self._collection, self);

  // Set up the filter data structures
  self.resetFilters();
  /*
  Fix for avoid reset on collection before it's fetched.
  [
  */
  var setListeners=function(){
    self.listenTo(self._superset, 'reset sort', execFilter);
    self.listenTo(self._superset, 'add change', onAddChange);
    self.listenTo(self._superset, 'remove', onModelRemove);
    self.listenTo(self._superset, 'all', onAll);
    }
  if(superset.hasOwnProperty("_p_fetched")){
    superset.onAfterFetch(function(){
      self._collection.add(superset.toArray());
      setListeners();
    });
  }else{
    setListeners();
